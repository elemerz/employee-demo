...
var helpers = {
  ...
  */
  ...
  /**
   * Usage: {@formatDate vale="{YOUR JS DATE VALUE}"/}
   * @param chunk
   * @param context
   * @param bodies
   * @param params
  ...
  "formatDate": function(chunk, context, bodies, params) {
	  var value = dust.helpers.tap(params.value, chunk, context),
	  		notime = !!dust.helpers.tap(params.notime, chunk, context),
	  		timestamp=new Date(Number(value)),
				year= timestamp.getFullYear(),
				month= padLeft(timestamp.getMonth() + 1,2), 
				date= padLeft(timestamp.getDate(),2),
				hours=padLeft(timestamp.getHours(),2),
				minutes=padLeft(timestamp.getMinutes(),2);
	  
	  return chunk.write(year + '.' + month + '.' + date + (!notime ? ' ' + hours + ':' + minutes : ''));
  },
  
  "dblEval": function(chunk, context, bodies, params) {
	  		var ctxName = dust.helpers.tap(params.context, chunk, context),
	  			value = dust.helpers.tap(params.value, chunk, context),
	  			result= context.get(ctxName)[value];
